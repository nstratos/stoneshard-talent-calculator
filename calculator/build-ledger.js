...
export default class BuildLedger {
    ...
    addAbility(abilityId) {
        ...
        const remainingAbilityPoints = this.totalAbilityPointsEarned - this.#countAllocations('ability');
    ...
    refundAbility(abilityId) {
        for (let lvl = this.level; lvl >= 1; lvl--) {
            const levelEntry = this.#levels[lvl];
            for (let i = levelEntry.allocations.length - 1; i >= 0; i--) {
                if (levelEntry.allocations[i].type === 'ability' && levelEntry.allocations[i].id === abilityId) {
                    levelEntry.allocations.splice(i, 1);
    ...
    refundAbilities(abilityIds) { 
        ...
        for (let lvl = this.level; lvl >= 1; lvl--) {
            const levelEntry = this.#levels[lvl];
            for (let i = levelEntry.allocations.length - 1; i >= 0; i--) {
                if (levelEntry.allocations[i].type === 'ability' && toRemove.has(levelEntry.allocations[i].id)) {
                    levelEntry.allocations.splice(i, 1);
                    removed++;
                }
            }
    ...
    static fromJSON(data) {
        ...
        const startingStatPoints = typeof data.startingStatPoints === 'number' ? data.startingStatPoints : 0;

        const startingAbilityPoints = typeof data.startingAbilityPoints === 'number' ? data.startingAbilityPoints : 2;
